...
class CMTrainLoop(TrainLoop):
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            elif self.training_mode == "consistency_td":
                compute_losses = functools.partial(
                    self.diffusion.consistency_td_losses,
                    self.ddp_model,
                    micro,
                    num_scales,
                    target_model=self.target_model,
                    ...
                    teacher_diffusion=self.teacher_diffusion,
                    model_kwargs=micro_cond,
                    lambda_td=getattr(self, "lambda_td", 0.5),
                    one_idx=getattr(self, "one_idx", 0.25),
                    weight_td=getattr(self, "weight_td", True),
                )
            else:
