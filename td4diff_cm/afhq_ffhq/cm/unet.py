...
class QKVFlashAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # flash-attn 2.x uses different import path
        try:
            from flash_attn.flash_attn_interface import flash_attn_func
            self.use_flash_attn_func = True
        except ImportError:
            # fallback for older versions
            from flash_attn.flash_attention import FlashAttention
            self.use_flash_attn_func = False
        ...
        if not self.use_flash_attn_func:
    ...
    def forward(self, qkv, attn_mask=None, key_padding_mask=None, need_weights=False):
        if self.use_flash_attn_func:
            # flash-attn 2.x API using flash_attn_func
            from flash_attn.flash_attn_interface import flash_attn_func
            qkv = self.rearrange(
            ...
            # Ensure tensors are contiguous for flash-attn backward pass
            q = q.contiguous()
            k = k.contiguous()
            v = v.contiguous()
            out = flash_attn_func(
            ...
            # flash-attn 1.x API using FlashAttention class
            qkv = self.rearrange(
                qkv, "b (three h d) s -> b s three h d", three=3, h=self.num_heads
            )
