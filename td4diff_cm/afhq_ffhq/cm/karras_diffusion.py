...
class KarrasDenoiser:
    ...
    def consistency_td_losses(
        ...
        is_boundary = (indices >= (num_scales - 2)).float()
        ...
        if self.loss_norm == "l1":
            diffs = th.abs(distiller - distiller_target)
            loss_ct = mean_flat(diffs) * weights
        elif self.loss_norm == "l2":
            diffs = (distiller - distiller_target) ** 2
            loss_ct = mean_flat(diffs) * weights
        elif self.loss_norm == "l2-32":
            ...
            if x_start.shape[-1] < 256:
                distiller_lpips = F.interpolate(distiller, size=224, mode="bilinear")
                distiller_target_lpips = F.interpolate(
                    distiller_target, size=224, mode="bilinear"
                )
            else:
                distiller_lpips = distiller
                distiller_target_lpips = distiller_target

            loss_ct = (
                self.lpips_loss(
                    (distiller_lpips + 1) / 2.0,
                    (distiller_target_lpips + 1) / 2.0,
                )
                * weights
            )
        else:
        ...
        def idx_to_sigma(idx):
            idx = th.clamp(idx, 0, num_scales - 1)
            t = self.sigma_max ** (1 / self.rho) + idx / (num_scales - 1) * (
                self.sigma_min ** (1 / self.rho) - self.sigma_max ** (1 / self.rho)
            )
            return t ** self.rho

        non_boundary_mask = 1.0 - is_boundary
        
        idx_t = indices.float()
        t_prev = idx_to_sigma(idx_t + one_idx)
        t_plus1_prev = idx_to_sigma(idx_t + 1.0 + one_idx)

        x_t_prev = (euler_solver if teacher_model is None else heun_solver)(
            x_t, t, t_prev, x_start
        ).detach()

        x_t_plus1 = (euler_solver if teacher_model is None else heun_solver)(
            x_t_prev, t_prev, t2, x_start
        ).detach()

        x_t_plus1_prev = (euler_solver if teacher_model is None else heun_solver)(
            x_t_plus1, t2, t_plus1_prev, x_start
        ).detach()

        f_t = distiller
        f_t_plus1 = target_denoise_fn(x_t_plus1, t2).detach()

        kappa_t = (t_prev / t) ** 2
        A_t = 1.0 - kappa_t
        kappa_t_plus1 = (t_plus1_prev / t2) ** 2
        A_t_plus1 = 1.0 - kappa_t_plus1

        mu_true_t = append_dims(A_t, dims) * x_start + append_dims(kappa_t, dims) * x_t
        mu_true_t_plus1 = (
            append_dims(A_t_plus1, dims) * x_start + append_dims(kappa_t_plus1, dims) * x_t_plus1
        )
        mu_model_t = append_dims(A_t, dims) * f_t + append_dims(kappa_t, dims) * x_t
        mu_model_t_plus1 = (
            append_dims(A_t_plus1, dims) * f_t_plus1 + append_dims(kappa_t_plus1, dims) * x_t_plus1
        )
        ...
        losses_td = mean_flat(losses_td)

        if weight_td:
            w_td = 1.0 / th.clamp(A_t_plus1 ** 2 + A_t ** 2, min=0.01)
            losses_td = losses_td * w_td

        loss_td = losses_td * non_boundary_mask

        loss_combined = (
            is_boundary * (1.0 + lambda_td) * loss_ct +
            non_boundary_mask * (loss_td + lambda_td * loss_ct)
        )

        terms = {}
        terms["loss"] = loss_combined
        terms["loss_ct"] = loss_ct.detach()
        terms["loss_td"] = loss_td.detach()
        terms["boundary_ratio"] = is_boundary.mean().detach()
