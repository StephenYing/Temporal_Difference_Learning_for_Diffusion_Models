...
def get_loss_fn(config, sde, score_model, state, rng):
    ...
    if config.training.loss.lower() in ["dsm", "ssm"]:
        ...
        if config.training.loss.lower() == "continuous_td":
            raise ValueError(
                "training.loss=continuous_td is not included in this release. Use consistency_td instead."
            )
        elif config.training.loss.lower().startswith("continuous"):
        ...
        elif config.training.loss.lower() == "consistency_td":
            train_loss_fn = get_consistency_td_loss_fn(
                sde,
                ref_model,
                ref_state.params_ema,
                ref_state.model_state,
                ...
                lpips_model=lpips_model,
                lpips_params=lpips_params,
                lambda_td=config.training.lambda_td,
                k_idx=config.training.k_idx,
                one_idx=config.training.one_idx,
                weight_td=config.training.get('weight_td', True),
                sigma_data=config.model.get('sigma_data', 0.5),
            )
            eval_loss_fn = get_consistency_td_loss_fn(
                sde,
                ref_model,
                ref_state.params_ema,
                ref_state.model_state,
...
def get_consistency_td_loss_fn(
    sde,
    ref_model,
    ref_params,
    ref_states,
    ...
    def idx_to_sigma(idx, num_scales):
        """Convert continuous index to sigma."""
        idx = jnp.clip(idx, 0, num_scales - 1)
        t = sde.t_max ** (1 / sde.rho) + idx / (num_scales - 1) * (
            sde.t_min ** (1 / sde.rho) - sde.t_max ** (1 / sde.rho)
        )
        return t ** sde.rho
    ...
    def loss_fn(rng, params, states, batch, target_params=None, num_scales=None):
        ...
        is_boundary = (indices >= (num_scales - 2))  
        
        sigma_t = sde.t_max ** (1 / sde.rho) + indices / (num_scales - 1) * (
            sde.t_min ** (1 / sde.rho) - sde.t_max ** (1 / sde.rho)
        )
        sigma_t = sigma_t ** sde.rho
        
        sigma_t_plus1 = sde.t_max ** (1 / sde.rho) + (indices + 1) / (num_scales - 1) * (
            sde.t_min ** (1 / sde.rho) - sde.t_max ** (1 / sde.rho)
        )
        sigma_t_plus1 = sigma_t_plus1 ** sde.rho
        
        idx_t = indices.astype(jnp.float32)
        idx_t_prev = idx_t + one_idx  
        idx_t_plus1 = idx_t + 1.0     
        idx_t_plus1_prev = idx_t_plus1 + one_idx  
        
        sigma_t_prev = idx_to_sigma(jnp.minimum(idx_t_prev, num_scales - 1), num_scales)
        sigma_t_plus1_prev = idx_to_sigma(jnp.minimum(idx_t_plus1_prev, num_scales - 1), num_scales)
        
        z = jax.random.normal(next(rng), x.shape)
        x_t = x + batch_mul(sigma_t, z)
        x_t_plus1_noised = x + batch_mul(sigma_t_plus1, z)  
        ...
        Ft, new_states = mutils.get_distiller_fn(
            sde, model, params, states, train=train, return_state=True
        )(x_t, sigma_t, rng=dropout_rng if train else None)

        x_t_plus1_ode = ode_solver(x_t, sigma_t, sigma_t_plus1, x)
        
        Ft_plus1, new_states = mutils.get_distiller_fn(
            sde, model, target_params, new_states, train=train, return_state=True
        )(x_t_plus1_ode, sigma_t_plus1, rng=dropout_rng if train else None)
        
        if stopgrad:
            Ft_plus1 = jax.lax.stop_gradient(Ft_plus1)

        diffs = Ft - Ft_plus1

        # CT weighting
        if weighting.lower() == "uniform":
            weight = jnp.ones_like(sigma_t)
        elif weighting.lower() == "snrp1":
            weight = 1 / sigma_t**2 + 1.0
        elif weighting.lower() == "truncated_snr":
            weight = jnp.maximum(1 / sigma_t**2, jnp.ones_like(sigma_t))
        elif weighting.lower() == "snr":
            weight = 1 / sigma_t**2
        else:
            raise NotImplementedError(f"Weighting {weighting} not implemented")
        ...
        # CT loss norm
        if loss_norm.lower() == "l1":
            losses_ct = jnp.abs(diffs)
            losses_ct = jnp.mean(losses_ct.reshape(losses_ct.shape[0], -1), axis=-1)
        elif loss_norm.lower() == "l2":
            losses_ct = diffs**2
            losses_ct = jnp.mean(losses_ct.reshape(losses_ct.shape[0], -1), axis=-1)
        elif loss_norm.lower() == "linf":
            losses_ct = jnp.abs(diffs)
            losses_ct = jnp.max(losses_ct.reshape(losses_ct.shape[0], -1), axis=-1)
        elif loss_norm.lower() == "lpips":
            scaled_Ft = jax.image.resize(
                Ft, (Ft.shape[0], 224, 224, 3), method="bilinear"
            )
            scaled_Ft_plus1 = jax.image.resize(
                Ft_plus1, (Ft_plus1.shape[0], 224, 224, 3), method="bilinear"
            )
            losses_ct = jnp.squeeze(lpips_model.apply(lpips_params, scaled_Ft, scaled_Ft_plus1))
        else:
            raise ValueError("Unknown loss norm: {}".format(loss_norm))

        loss_ct = jnp.nansum(losses_ct * batch["mask"] * weight / jnp.sum(batch["mask"]))

        # TD Loss
        non_boundary_mask = (~is_boundary).astype(jnp.float32)
    
        x_t_prev = ode_solver(x_t, sigma_t, sigma_t_prev, x)
        x_t_plus1_ode = ode_solver(x_t_prev, sigma_t_prev, sigma_t_plus1, x)
        x_t_plus1_prev = ode_solver(x_t_plus1_ode, sigma_t_plus1, sigma_t_plus1_prev, x)
        f_t = Ft
        
        f_t_plus1, _ = mutils.get_distiller_fn(
            sde, model, target_params, new_states, train=train, return_state=True
        )(x_t_plus1_ode, sigma_t_plus1, rng=dropout_rng if train else None)
        f_t_plus1 = jax.lax.stop_gradient(f_t_plus1)
        
        kappa_t = (sigma_t_prev / sigma_t) ** 2
        A_t = 1.0 - kappa_t
        
        kappa_t_plus1 = (sigma_t_plus1_prev / sigma_t_plus1) ** 2
        A_t_plus1 = 1.0 - kappa_t_plus1
        
        mu_true_t = batch_mul(A_t, x) + batch_mul(kappa_t, x_t)
        mu_true_t_plus1 = batch_mul(A_t_plus1, x) + batch_mul(kappa_t_plus1, x_t_plus1_ode)
        
        mu_model_t = batch_mul(A_t, f_t) + batch_mul(kappa_t, x_t)
        mu_model_t_plus1 = batch_mul(A_t_plus1, f_t_plus1) + batch_mul(kappa_t_plus1, x_t_plus1_ode)
        
        delta_true = mu_true_t_plus1 - mu_true_t
        delta_model = mu_model_t_plus1 - mu_model_t
        
        losses_td = (delta_true - delta_model) ** 2
        losses_td = jnp.mean(losses_td.reshape(losses_td.shape[0], -1), axis=-1)
        
        if weight_td:
            # Weight: 1 / (A_{t+1} + A_t)
            w_td = 1.0 / jnp.maximum(A_t_plus1 + A_t, 0.01)
            w_td = w_td.squeeze()
            losses_td = losses_td * w_td
        
        losses_td = losses_td * non_boundary_mask
        
        # Total Loss
        is_boundary_float = is_boundary.astype(jnp.float32)
        losses_ct_weighted = losses_ct * weight  

        # Boundary samples: (1 + λ) * CT_loss
        # Non-boundary samples: TD_loss + λ * CT_loss
        losses_combined = (
            is_boundary_float * (1.0 + lambda_td) * losses_ct_weighted +
            (1.0 - is_boundary_float) * (losses_td + lambda_td * losses_ct_weighted)
        )
        
        loss = jnp.nansum(losses_combined * batch["mask"] / jnp.sum(batch["mask"]))
        loss_ct = jnp.nansum(losses_ct_weighted * batch["mask"] / jnp.sum(batch["mask"]))
        loss_td = jnp.nansum(losses_td * batch["mask"] / jnp.sum(batch["mask"]))
        
        log_stats = {
            "loss_ct": loss_ct,
            "loss_td": loss_td,
            "boundary_ratio": jnp.mean(is_boundary.astype(jnp.float32)),
        }
